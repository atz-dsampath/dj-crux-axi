...
export const PLAYBOOKS = [
  {
    ...
    design_rules: [
      ...
      "Initialize Mermaid to match the page theme and re-render when the theme changes: pick the Mermaid theme from the effective page appearance (light or dark) at render time, and use the theme-aware `crux-axi design` Mermaid snippet rather than hardcoding a single theme, since Mermaid does not restyle an already-rendered SVG when the viewer toggles the page theme.",
    ],
    pitfalls: [
    ...
    crux_notes: [
      "A Crux diagram should invite precise annotation: make modules, edges, and captions easy to click and discuss.",
  ...
  },
  {
    ...
    crux_notes: [
      "A Crux table should make individual rows easy annotation targets.",
  ...
  },
  {
    ...
    crux_notes: [
      "A Crux comparison should let the user annotate the exact option or tradeoff they want changed.",
  ...
  },
  {
    ...
    crux_notes: ["A Crux plan should make a plan and its uncertainties easy to annotate before code exists."],
  },
  {
    id: "code",
    use_when: "Render source code, code files, patches, PR diffs, and before/after code inside Crux artifacts",
    choose: [
...
\`\`\``,
    ...
    crux_notes: [
      "A Crux code artifact should make each file, hunk, and relevant line easy to annotate precisely.",
  ...
  },
  {
    ...
    design_rules: [
      "Native controls - radios, checkboxes, text inputs, selects, textareas, buttons, options, labels, disclosure summaries, and contenteditable regions - are interactive automatically: clicks toggle, focus, and type instead of annotating, so they do not need data-crux-action. Build choice and option UIs from these whenever you can.",
      "For reversible choices, do not call window.crux.queuePrompt() from radio change handlers or option click handlers. Those handlers should only update local selected state.",
      "Use a per-question form submit or explicit Queue answer button to read the current values and call window.crux.queuePrompt() exactly once for the final answer.",
      "Put data-crux-action only on custom (non-native) elements that should act like a feedback control - typically a styled div or span you made clickable - so Crux does not annotate it and shows a pointer cursor instead.",
      "Use data-crux-question on a question wrapper or pass queueKey when multiple pre-send updates should replace the prior unsent answer for the same question.",
      ...
      "Call window.crux.sendQueuedPrompts() only when the control should immediately send committed feedback instead of waiting for the user to press Send to Agent.",
    ...
    crux_notes: [
      "Crux is strongest when the artifact becomes a focused review surface and not just a static page.",
      'A native single-choice question should submit the final value: `<form data-crux-question="plan" onsubmit="event.preventDefault(); const choice = new FormData(event.currentTarget).get(\'plan\'); if (choice) window.crux.queuePrompt(\'Use the \' + choice + \' plan\', { tag: \'choice\', text: \'Plan: \' + choice, element: event.currentTarget, data: { question: \'plan\', answer: choice } });"><label><input type="radio" name="plan" value="Starter"> Starter</label><label><input type="radio" name="plan" value="Pro"> Pro</label><button type="submit">Queue this answer</button></form>`.',
      "A custom choice UI should make option buttons update local state, then use a separate Queue answer button with data-crux-action to queue the final selected value.",
      "Use window.crux.queuePrompt for user intent, not internal analytics or UI-only state changes.",
  ...
  },
  {
    ...
    crux_notes: [
      "A Crux slide deck can still collect feedback, but each prompt should refer to a slide or decision.",
