...
// Builds a portable copy of a Crux artifact by inlining only its LOCAL assets - files on disk
...
export async function buildSelfContainedHtml(html, options = {}) {
  ...
  const ctx = {
    ...
    maxAssetBytes: resolveBytes(
      ...
      process.env.CRUX_AXI_EXPORT_MAX_ASSET_BYTES,
      DEFAULT_MAX_ASSET_BYTES,
    ),
    maxBundleBytes: resolveBytes(
      options.maxBundleBytes,
      process.env.CRUX_AXI_EXPORT_MAX_BUNDLE_BYTES,
...
async function inlineScript(tag, attrs, body, closeTag, baseDir, ctx) {
  ...
  if (isInjectedCruxSdkSrc(src)) return "";
  if (isModuleScript(attrs)) {
...
function isInjectedCruxSdkSrc(src) {
