...
const DESCRIPTION =
  "Crux Editor helps agents turn rich HTML artifacts into collaborative human review surfaces. Whenever you are about to give user a complex response that will be easier to understand via a rich / interactive page, consider using Crux Editor. " +
  "First generate an interactive HTML artifact according to user request, then run `crux-axi <html-file>` so the user can visually review it, annotate elements or selected text, queue prompts, and send feedback back through `crux-axi poll`.";
...
export const VERSION =
  process.env.CRUX_AXI_BUILD_VERSION ||
...
export function createHomeOutput({ bin, sessions, includeSessions = true, agent = "generic" }) {
  return {
    ...
    help: [
      "Run `crux-axi <html-file>` to open or resume a Crux Editor session. If the user explicitly ended the session from the browser, this refuses to reopen it and explains why instead of reopening uninvited - pass `--reopen` only when the user asks for further review or something important needs their visual attention",
      "Unless the user specifies another location, create HTML artifacts in the current working directory under `.crux/`",
      ...
      "Run `crux-axi end <html-file>` to end a session as the agent - ending it this way still allows a plain reopen later. When the user ends it from the browser instead, a later `crux-axi <html-file>` refuses to reopen it without `--reopen`",
      "Run `crux-axi export <html-file> [--out <path>]` to write a portable copy of the artifact - one HTML file with its LOCAL assets inlined - so it opens with no Crux server and no sibling files. Remote CDN/font references are left as links, so it needs network to render those. Users can also export from the browser chrome's overflow menu",
      "Run `crux-axi share <html-file> [--password <pw>] [--token <t>]` to publish the artifact on ht-ml.app (https://ht-ml.app), a third-party hosting service not part of Crux, and get back a visitable URL. Shares are PUBLIC by default, so anyone with the link can open them. Pass --password to publish a PRIVATE password-protected page; viewers must supply the password to view. Local assets are inlined; remote refs load over the network. It returns the url plus a secret update_key for managing the page later. Use --token or CRUX_AXI_HTML_APP_TOKEN only when you have an optional bearer token; it is never required. Users can also publish from the browser chrome's overflow menu",
      "Run `crux-axi stop` to shut down the background server (it also self-stops when idle or after the last session ends with nothing connected)",
      `Run \`crux-axi playbook <playbook_id>\` for focused artifact guidance. ${PLAYBOOK_ROUTER_HELP}`,
      DESIGN_SYSTEM_HINT,
      "Use crux-axi when the user asks for a visual artifact, HTML explainer, interactive prototype, review surface, product or technical plan, comparison, report, or browser-based feedback loop",
    ],
  };
}
...
export function createPlaybookOutput(args) {
  ...
  if (!id) {
    return {
      playbooks: listPlaybooks(),
      help: ["Run `crux-axi playbook <playbook_id>` for focused artifact guidance", PLAYBOOK_ROUTER_HELP],
    };
  }
  ...
  if (!playbook) {
    throw new AxiError(`Unknown playbook: ${id}`, "VALIDATION_ERROR", [
      `Run \`crux-axi playbook\` to list known IDs: ${playbookIds().join(", ")}`,
    ]);
  }
...
export function createOpenOutput({ file, url, status, agent = "generic" }) {
  return {
    ...
    next_step: `Do not respond to the user just yet. Now you must run \`crux-axi poll ${file}\`. This command long-polls until the user sends feedback, ends the session, or the real browser proves a severe layout failure through the in-iframe audit, and it stays silent the whole time - that is normal, never kill it. If layout_warnings arrive, follow the poll response's next_step: repair and re-check the inaccessible or unusable content before involving the human. Cosmetic, intentional, transient, tiny, and uncertain observations stay silent. Do not pass --timeout-ms during normal agent use. ${pollExecutionGuidance({ agent })} After applying feedback, run \`crux-axi poll ${file} --agent-reply "<message for the user>"\` without --timeout-ms to show your response in Crux Editor and wait for more feedback. If the user ends the session, stop polling and do not reopen it by re-running \`crux-axi ${file}\` unless the user asks for further review or something genuinely important needs their visual attention - deliver routine updates directly in this conversation instead. When reopening is warranted, run \`crux-axi ${file} --reopen\`.`,
  };
}

// Shown when a plain `crux-axi <file>` targets a session the user explicitly ended from the
...
export function createUserEndedOpenOutput({ file, url }) {
  return {
    ...
    next_step: `The user explicitly ended this Crux Editor session from the browser, so \`crux-axi ${file}\` did not reopen it. Do not reopen unless the user asks for further review or something genuinely important needs their visual attention - deliver routine updates directly in this conversation instead. When reopening is warranted, run \`crux-axi ${file} --reopen\`.`,
  };
}
...
async function openCommand(args) {
  ...
  if (!file) {
    throw new AxiError("HTML file path is required", "VALIDATION_ERROR", ["Run `crux-axi <html-file>`"]);
  }
...
export function shouldOpenBrowser(args, env) {
  return !args.includes("--no-open") && env.CRUX_AXI_NO_OPEN !== "1";
}

async function pollCommand(args) {
  const file = firstPositionalArg(args, ["--agent-reply", "--timeout-ms"]);
  if (!file) {
    throw new AxiError("HTML file path is required", "VALIDATION_ERROR", ["Run `crux-axi poll <html-file>`"]);
  }
  const absolute = await canonicalFile(file);
...
export function pollWaitBannerText(file) {
  return (
    `[crux-axi] Long-polling for user feedback or layout_warnings on ${file}. This stays silent until the user sends feedback, ends the session, or the browser reports fresh layout_warnings - leave it running. ` +
    `If it gets killed or times out, re-run \`crux-axi poll ${file}\` - queued feedback is never lost.`
  );
}

export function pollWaitTickText(elapsedMs) {
  const minutes = Math.round(elapsedMs / 60_000);
  return `[crux-axi] Still waiting for user feedback (${minutes}m). Also waiting for fresh layout_warnings. Leave this running until the user acts or the browser reports fresh layout_warnings.`;
}

export function pollInterruptedText(file) {
  return (
    `[crux-axi] Poll interrupted before user feedback arrived. The user may still be reviewing - ` +
    `re-run \`crux-axi poll ${file}\` to keep waiting; queued feedback is never lost.`
  );
}
...
export function createPollOutput({ file, response, agent = "generic" }) {
  if (response.status === "missing") {
    throw new AxiError("No active Crux Editor session for this file", "NOT_FOUND", [`Run \`crux-axi ${file}\` first`]);
  }
  ...
  return {
    ...
    next_step: `No user feedback arrived before the optional timeout. Run \`crux-axi poll ${file}\` without --timeout-ms to wait indefinitely - queued feedback is never lost, so re-running the poll is always safe.`,
  };
}
...
function createFeedbackNextStep(file, layoutWarnings, sessionEnded, endedBy, prompts = [], agent = "generic") {
  ...
  const whiteboardNote = prompts.some((prompt) => prompt && prompt.tag === "whiteboard")
    ? `This feedback includes whiteboard edits (tag "whiteboard"): read the edit summary in the prompt text first, and only when it is not enough, open the target's scenePath (.excalidraw scene JSON) or previewPath (PNG) local files for detail. The artifact's Mermaid source stays authoritative - apply the edits by updating the Mermaid text in ${file} (Crux live-reloads it); never try to write the .excalidraw scene back. `
    : "";
  if (sessionEnded) {
    const layoutNote =
      count > 0
        ? endedBy === "user"
          ? `${count} proven severe layout failure${count === 1 ? "" : "s"} arrived alongside this final feedback. Repair the inaccessible or unusable content in ${file}, then open it directly at the affected viewport and confirm the content or control is visible and usable without reopening this ended Crux session. `
          : `${count} proven severe layout failure${count === 1 ? "" : "s"} arrived alongside this final feedback. Repair the inaccessible or unusable content in ${file}, then run \`crux-axi ${file}\` to open a fresh session and re-check the real-browser audit. `
        : "";
    if (endedBy === "user") {
      const reopenNote =
        count > 0
          ? ""
          : ` Only run \`crux-axi ${file} --reopen\` if the user explicitly asks for further review or something genuinely important needs their visual attention.`;
    ...
    return `${layoutNote}${whiteboardNote}This was the last feedback before the Crux Editor session ended. Stop polling ${file}. Deliver any remaining updates directly in this conversation, or run \`crux-axi ${file}\` to open a fresh session if the user needs further visual review.`;
  }
  const layoutPrefix =
    count > 0 ? layoutWarningsPrefix(file, layoutWarnings) : `Apply the requested changes to ${file}. `;
  return `${layoutPrefix}${whiteboardNote}Do not respond to the user just yet. Now you must run \`crux-axi poll ${file} --agent-reply "<message for the user>"\` without --timeout-ms unless the user ended the session. The poll waits silently until the user sends more feedback, ends the session, or reports fresh layout_warnings - never kill it. ${pollExecutionGuidance({ agent })}`;
}
...
function layoutWarningsPrefix(file, layoutWarnings) {
  ...
  return `${count} proven severe layout failure${plural} detected - repair the inaccessible or unusable content in ${file}, then re-check in the browser before involving the human. Crux live-reloads the artifact automatically after you save, so you do not need to re-run \`crux-axi ${file}\` for this. `;
}

function createEndedNextStep(file, endedBy) {
  if (endedBy === "user") {
    return `The user ended this Crux Editor session. Stop polling ${file} - do not run \`crux-axi ${file}\` to reopen it. Deliver any remaining updates directly in this conversation instead. Only reopen with \`crux-axi ${file} --reopen\` if the user explicitly asks for further review or something genuinely important needs their visual attention.`;
  }
  return `This Crux Editor session for ${file} has ended. Stop polling. Deliver any remaining updates directly in this conversation, or run \`crux-axi ${file}\` to open a fresh session if the user needs further visual review.`;
}

async function endCommand(args) {
  const file = firstPositionalArg(args);
  if (!file) {
    throw new AxiError("HTML file path is required", "VALIDATION_ERROR", ["Run `crux-axi end <html-file>`"]);
  }
  const absolute = await canonicalFile(file);
  const baseUrl = await ensureServer();
...
// as-is for the browser to load, so the export needs network to render those. Crux makes no
...
async function exportCommand(args) {
  ...
  if (!file) {
    throw new AxiError("HTML file path is required", "VALIDATION_ERROR", ["Run `crux-axi export <html-file>`"]);
  }
  await assertHtmlFile(file);
  const absolute = await canonicalFile(file);
...
export function createExportOutput({ source, output, html, warnings }) {
  ...
  if (unresolved.length) {
    ...
    result.next_step = `Wrote ${output} with export notices (see notices). Open it directly or host it anywhere - it needs no Crux server. Local assets are inlined; remote CDN/font references are left as links, so it needs network to render those.`;
  } else {
    result.next_step = `Wrote ${output}. Open it directly or host it anywhere - it needs no Crux server. Local assets are inlined; remote CDN/font references are left as links, so it needs network to render those.`;
  }
  return result;
}
...
// sending the artifact to ht-ml.app's servers. The service is not part of Crux, needs no
...
async function shareCommand(args) {
  ...
  if (!file) {
    throw new AxiError("HTML file path is required", "VALIDATION_ERROR", ["Run `crux-axi share <html-file>`"]);
  }
  await assertHtmlFile(file);
  const absolute = await canonicalFile(file);
...
export function createShareOutput({ source, site, warnings, passwordProtected = false }) {
  ...
  const hostNote =
    "ht-ml.app (https://ht-ml.app), a third-party host not part of Crux, hosts the page, so it needs no Crux server.";
  if (unresolved.length) {
    result.next_step =
...
// Explicitly shut down the running Crux Editor server. Unlike `end` (which closes a single
...
export async function shutdownServerOnPort(
  port,
  {
    ...
    processMatchesCrux = processOnPortMatchesCrux,
  } = {},
) {
  ...
  if (!(await canControlServerOnPort(port, health, processMatchesCrux))) {
    return { server: { status: "not-crux", port } };
  }
...
async function setupCommand(args) {
  if (args.length !== 1 || args[0] !== "hooks") {
    throw new AxiError("Unknown setup action", "VALIDATION_ERROR", ["Run `crux-axi setup hooks`"]);
  }

  const errors = [];
  installSessionStartHooks({
    marker: "crux-axi",
    binaryNames: ["crux-axi"],
    distEntrypoints: ["dist/cli.mjs", "bin/crux-axi.js"],
  ...
  if (errors.length > 0) {
    throw new AxiError("Failed to install crux-axi agent hooks", "SERVER_ERROR", errors);
  }

  return {
    hooks: { status: "installed", integrations: "Claude Code, Codex, OpenCode, GitHub Copilot CLI" },
    help: ["Restart your agent session to receive crux-axi ambient context"],
  };
}
...
export function createCopilotCliAmbientContextScript(command = "crux-axi") {
  return [
    ...
    'const prefix = result.status === 0 ? "## AXI ambient context: crux-axi\\n" : "## AXI ambient context: crux-axi\\nerror: crux-axi ambient context failed: ";',
    "console.log(JSON.stringify({ additionalContext: prefix + text }));",
  ].join(" ");
}

export function createCopilotCliSessionStartHook(command = "crux-axi", timeoutSec = 10) {
...
export function installCopilotCliSessionStartHook({
  ...
  const target = path.join(hookDir, "crux-axi.json");
  try {
...
function isManagedCopilotCliHook(entry) {
  return (
    ...
    [entry.bash, entry.powershell, entry.command].some(
      (value) => typeof value === "string" && value.includes("crux-axi"),
    )
  );
}
...
async function serverCommand(args) {
  const port = Number(flagValue(args, "--port") || defaultPort());
  const debug = args.includes("--verbose") || process.env.CRUX_AXI_DEBUG === "1";
...
async function assertHtmlFile(file) {
  if (!isHtmlPath(file)) {
    throw new AxiError("Crux Editor expects an HTML file", "VALIDATION_ERROR", ["Run `crux-axi <html-file>`"]);
  }
  try {
    await access(file);
  } catch {
    throw new AxiError(`File not found: ${file}`, "NOT_FOUND", [
      "Create the HTML artifact first, then run `crux-axi <html-file>`",
    ]);
  }
}
...
async function ensureServer({ forceRestart = false } = {}) {
  ...
  if (existing) {
    if (!(await canControlServerOnPort(port, existing, processOnPortMatchesCrux))) {
      throw new AxiError(`Port ${port} is occupied by a non-Crux server`, "SERVER_ERROR", [
        `Stop the process using port ${port}, or set CRUX_AXI_PORT to another port`,
      ]);
    }
  ...
  throw new AxiError("Crux Editor server did not start", "SERVER_ERROR", [
    `Run \`crux-axi server --port ${port}\` to inspect server startup`,
  ]);
}
...
export function shouldKillProcessOnPort(currentVersion, healthBody) {
  if (!healthBody || typeof healthBody !== "object") return false;
  if (typeof healthBody.version !== "string" || healthBody.version === "") return true;
  if (healthBody.app !== "crux-axi") return false;
  return healthBody.version !== currentVersion;
}

async function canControlServerOnPort(port, healthBody, processMatchesCrux) {
  if (!healthBody || typeof healthBody !== "object") return false;
  if (healthBody.app === "crux-axi") return true;
  if (typeof healthBody.version === "string" && healthBody.version !== "") return false;
  return processMatchesCrux(port);
}
...
// macOS/Linux only - Windows users would need to kill manually, but crux-axi isn't
...
function processOnPortMatchesCrux(port) {
  try {
    ...
    for (const line of pids.stdout.split("\n")) {
      ...
      if (command.status === 0 && /crux-axi/.test(command.stdout)) {
        return true;
      }
    }
  } catch {
...
// run(). In source layout that's `../bin/crux-axi.js` (which calls run on import). In the
...
export function resolveServerEntry() {
  const binEntry = fileURLToPath(new URL("../bin/crux-axi.js", import.meta.url));
...
export function createServerSpawnOptions(logFd = null) {
  ...
  return {
    detached: true,
    stdio,
    env: { ...process.env, CRUX_AXI_NO_OPEN: "1" },
  };
}
...
export async function fetchJson(url, { retries = 0, retryDelayMs = 250 } = {}) {
  ...
  if (!response.ok) {
    throw new AxiError(`Crux Editor request failed: ${response.status}`, "SERVER_ERROR");
  }
  try {
    return await response.json();
  } catch {
...
function serverConnectionError() {
  return new AxiError("Crux Editor server connection failed", "SERVER_ERROR", [
    "Run `crux-axi server --verbose` or inspect `~/.crux-axi/server.log` (`CRUX_AXI_STATE_DIR/server.log` when set) for server startup or crash diagnostics",
    "Re-run the last `crux-axi poll <html-file>` command after the server is healthy",
  ]);
}

function pollResponseInterruptedError() {
  return new AxiError("Crux Editor poll response was interrupted", "SERVER_ERROR", [
    "Run `crux-axi server --verbose` or inspect `~/.crux-axi/server.log` (`CRUX_AXI_STATE_DIR/server.log` when set) for server startup or crash diagnostics",
    "Re-run the last `crux-axi poll <html-file>` command after the server is healthy",
  ]);
}
...
function createTopLevelHelp({ agent = "generic" } = {}) {
  return `crux-axi - Crux Editor AXI\n\nUsage:\n  crux-axi\n  crux-axi <html-file> [--no-open] [--no-gate] [--reopen]\n  crux-axi poll <html-file> [--agent-reply "..."]\n  crux-axi end <html-file>\n  crux-axi export <html-file> [--out <path>]\n  crux-axi share <html-file> [--password <pw>] [--token <t>]\n  crux-axi stop\n  crux-axi playbook [playbook_id]\n  crux-axi design\n  crux-axi setup hooks\n\n${DESIGN_SYSTEM_HINT}\n\nNote: poll long-polls indefinitely by default until the user sends feedback, ends the session, or the browser proves a severe layout failure, staying silent while it waits - never kill it. Repair and re-check every returned layout failure before involving the human; cosmetic and uncertain observations are never returned. Do not pass --timeout-ms during normal agent use; it is for tests and debugging only. ${pollExecutionGuidance({ agent })} ${POLL_SEND_AND_END_RULE}\n\n`;
}

function createCommandHelp({ agent = "generic" } = {}) {
  return {
    open: `Usage: crux-axi <html-file> [--no-open] [--no-gate] [--reopen]\n\nOpen or resume a Crux Editor review session for an HTML artifact. Use --no-open when you need to ensure the server/session exists without opening another browser window. Use --no-gate to skip the open-time layout curtain for this browser open. If the user explicitly ended the session from the browser, this refuses to reopen it and returns guidance instead - pass --reopen to force it open when the user asks for further review or something important needs their visual attention. Sessions ended by the agent (\`crux-axi end\`) reopen normally without the flag.\n`,
    poll: `Usage: crux-axi poll <html-file> [--agent-reply "..."]\n\nThis command long-polls indefinitely for queued user prompts and browser-proven severe layout failures, then returns them to the agent as layout_warnings. It stays silent while it waits - that is normal, never kill it. Repair and re-check every returned layout failure before involving the human; cosmetic and uncertain observations are never returned. Do not pass --timeout-ms during normal agent use; it is for tests and debugging only. ${pollExecutionGuidance({ agent })} Use --agent-reply after applying prior feedback to display your response in Crux Editor before waiting again. ${POLL_SEND_AND_END_RULE}\n`,
    end: `Usage: crux-axi end <html-file>\n\nEnd a Crux Editor session as the agent. A session ended this way still reopens normally on the next \`crux-axi <html-file>\`, unlike a user ending it from the browser, which requires --reopen.\n`,
    export: `Usage: crux-axi export <html-file> [--out <path>]\n\nWrite a portable copy of an artifact: one HTML file with its LOCAL assets inlined (relative-path stylesheets, scripts, images, and fonts become inline <style>/<script> blocks and data URIs). Remote CDN/font references (https URLs) are left as links for the browser to load, so the file needs network to render those. Crux makes no outbound requests - it only reads local files, confined to the artifact's directory. Defaults to writing <name>.export.html next to the source; pass --out to choose a path. The Crux annotation SDK is never included in an export.\n`,
    share: `Usage: crux-axi share <html-file> [--password <pw>] [--token <t>]\n\nPublish the artifact on ht-ml.app (https://ht-ml.app), a third-party hosting service not part of Crux, and print a visitable URL. Shares are PUBLIC by default: anyone with the link can open the page, and it may be indexed or scraped. Pass --password to publish a PRIVATE password-protected page; viewers must supply the password to view. Builds the same local-inlined HTML as 'export' (local assets inlined; remote CDN/font URLs left as links and are not blocked by CSP on ht-ml.app, but still load over the viewer's network), then POSTs it to ht-ml.app's /v1 API. Creating a site needs no account or API key. The response includes the url plus a secret update_key (shown once) for updating or deleting the page later. Set CRUX_AXI_HTML_APP_TOKEN (or pass --token) to attach an optional bearer token; it is never required. The annotation SDK is never included.\n`,
    stop: `Usage: crux-axi stop [--port <port>]\n\nShut down the background Crux Editor server. The server also stops itself when no browser or poll has been connected for a while (CRUX_AXI_IDLE_TIMEOUT_MS, default 30m) and immediately when the last session ends with nothing connected.\n`,
