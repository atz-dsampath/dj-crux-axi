...
export function resolveTelemetryConfig(input) {
  const optOut = String(input.env.CRUX_AXI_TELEMETRY || "")
    .trim()
    .toLowerCase();
  ...
  const websiteID = String(input.env.CRUX_AXI_UMAMI_WEBSITE_ID || "").trim() || input.buildWebsiteID.trim();
  if (!websiteID) {
    return { enabled: false, host: "", websiteID: "" };
  }
...
export function getBuildTimeUmamiHost() {
  return process.env.CRUX_AXI_BUILD_UMAMI_HOST || "";
}

export function getBuildTimeUmamiWebsiteID() {
  return process.env.CRUX_AXI_BUILD_UMAMI_WEBSITE_ID || "";
}
