...
Task: make Mermaid diagrams in Crux artifacts respect the artifact page theme instead of
rendering in a fixed theme.

All screenshots were captured from the real browser view through the real `crux-axi` flow
...
## AFTER (theme-aware init from `crux-axi design`)
...
`crux-axi <file>` and toggle Dark/Light.
