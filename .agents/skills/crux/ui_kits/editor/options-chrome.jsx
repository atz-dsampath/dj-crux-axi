...
// Each renders a small, static Crux Editor chrome at artboard scale.
...
const mc = {
  ...
  divider: { width: 1, height: 18, background: "var(--border-subtle)", flexShrink: 0 },
  fileWrap: { display: "flex", alignItems: "center", gap: 5, flex: 1, minWidth: 0, color: "var(--fg-dim)", fontFamily: "var(--font-mono)", fontSize: 10.5 },
  ...
  panel: { width: 234, flexShrink: 0, borderLeft: "1px solid var(--border-subtle)", background: "var(--bg-panel)", display: "flex", flexDirection: "column", minHeight: 0 },
  ...
  ta: { width: "100%", minHeight: 52, borderRadius: 10, border: "1px solid var(--border)", background: "var(--bg)", color: "var(--fg)", padding: 8, font: "inherit", fontSize: 11, boxSizing: "border-box", resize: "none" },
};

const ghostBtn = { border: "1px solid var(--border)", background: "transparent", color: "var(--fg-muted)", borderRadius: 9, padding: "6px 10px", fontFamily: "inherit", fontWeight: 600, fontSize: 11, cursor: "pointer", whiteSpace: "nowrap" };
const filledBtn = { border: 0, background: "var(--accent)", color: "var(--accent-ink)", borderRadius: 9, padding: "6px 11px", fontFamily: "inherit", fontWeight: 700, fontSize: 11, cursor: "pointer", whiteSpace: "nowrap" };
const iconBtn = { width: 28, height: 28, display: "grid", placeItems: "center", border: "1px solid transparent", background: "transparent", color: "var(--fg-dim)", borderRadius: 8, cursor: "pointer", flexShrink: 0, padding: 0 };
...
function Brand() {
  return (
    <div style={mc.brand}>
      <span style={mc.brandMark}>Crux</span>
...
function Toggle({ on = true }) {
  return (
    <div style={{ display: "inline-flex", alignItems: "center", gap: 6, color: "var(--fg-dim)", fontSize: 10.5, whiteSpace: "nowrap" }}>
      <span style={{ width: 26, height: 15, borderRadius: 999, background: on ? "var(--accent)" : "var(--border)", position: "relative", flexShrink: 0, transition: "background .15s" }}>
...
function ArtifactBody({ note }) {
  return (
    <div style={mc.artifact}>
      ...
      <div style={{ ...mc.artH1, outline: "2px solid var(--accent)", outlineOffset: 2 }}>For when a rich editor<br/>is not rich enough.</div>
      <p style={mc.artLede}>Crux opens an agent-generated HTML artifact in a local browser, lets you pinpoint elements, and ships your feedback back to the agent.</p>
      {note}
    </div>
  );
}
  ...
  return (
    <div style={mc.shell}>
      <div style={mc.bar}>
        ...
        <button style={mc.barBtn || { ...ghostBtn, background: "var(--border-subtle)", border: 0 }}>Annotation: On</button>
        <button style={{ ...ghostBtn, color: "var(--danger)", borderColor: "var(--danger)" }}>End Session</button>
      </div>
      <div style={mc.body}>
        <ArtifactBody note={
          <div style={{ position: "absolute", left: 150, top: 92, width: 188, padding: 9, borderRadius: 11, background: "var(--bg-panel)", color: "var(--fg)", border: "1px solid var(--accent)", boxShadow: "var(--shadow-floating)" }}>
            <div style={{ fontWeight: 700, marginBottom: 5, fontSize: 11 }}>Annotate &lt;h1&gt;</div>
            <div style={{ height: 30, borderRadius: 7, border: "1px solid var(--border)", background: "var(--bg)" }} />
        ...
        <div style={mc.panel}>
          ...
          <Conversation extra={
            <div style={{ display: "flex", flexWrap: "wrap", gap: 4 }}>
              <span style={{ display: "inline-flex", alignItems: "center", gap: 4, border: "1px solid var(--border-strong)", borderRadius: 999, background: "var(--bg-elevated)", padding: "3px 5px 3px 8px", fontSize: 9.5, fontWeight: 700 }}>Make CTA italic<span style={{ width: 13, height: 13, borderRadius: 999, background: "var(--border)", display: "grid", placeItems: "center", fontSize: 10 }}>×</span></span>
            </div>
          } />
        ...
        <ArtifactBody note={
          <div style={{ position: "absolute", left: 150, top: 92, width: 188, padding: 9, borderRadius: 11, background: "var(--bg-panel)", color: "var(--fg)", border: "1px solid var(--accent)", boxShadow: "var(--shadow-floating)" }}>
            ...
            <div style={{ display: "flex", gap: 5, justifyContent: "flex-end", marginTop: 6, alignItems: "center" }}>
              <span style={{ fontSize: 10, color: "var(--fg-faint)", cursor: "pointer" }}>Cancel</span>
  ...
  return (
    <div style={mc.shell}>
      <div style={mc.bar}>
        <Brand /><div style={mc.divider} /><FileChip />
        <Toggle on={true} />
        <button title="More" style={{ ...iconBtn, border: "1px solid var(--border-subtle)" }}><Ic d={moreIcon} /></button>
      </div>
      <div style={mc.body}>
        <ArtifactBody note={
          <div style={{ position: "absolute", left: 150, top: 92, width: 188, padding: 9, borderRadius: 11, background: "var(--bg-panel)", color: "var(--fg)", border: "1px solid var(--accent)", boxShadow: "var(--shadow-floating)" }}>
            ...
            <div style={{ height: 30, borderRadius: 7, border: "1px solid var(--border)", background: "var(--bg)" }} />
            <div style={{ display: "flex", gap: 5, justifyContent: "flex-end", marginTop: 6, alignItems: "center" }}>
              <span style={{ fontSize: 10, color: "var(--fg-faint)" }}>Esc</span>
              <button style={{ ...filledBtn, padding: "4px 9px", fontSize: 10 }}>Queue</button>
            </div>
          </div>
        } />
    ...
    <div style={mc.shell}>
      ...
      <div style={mc.body}>
        ...
        <div style={mc.panel}>
          ...
          <Conversation />
          <div style={mc.composer}>
            <div style={{ display: "inline-flex", alignItems: "center", gap: 5, alignSelf: "flex-start", border: "1px solid var(--accent)", color: "var(--accent)", borderRadius: 999, padding: "3px 5px 3px 9px", fontSize: 9.5, fontWeight: 700, fontFamily: "var(--font-mono)" }}>
