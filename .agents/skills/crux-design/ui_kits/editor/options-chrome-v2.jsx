...
const v2 = {
  shell: { position: "absolute", inset: 0, display: "flex", flexDirection: "column", background: "var(--bg)", color: "var(--fg)", fontFamily: "var(--font-sans)", fontSize: 12, overflow: "hidden" },
  bar: { height: 44, display: "flex", alignItems: "center", gap: 12, padding: "0 12px", background: "var(--bg-bar)", borderBottom: "1px solid var(--border-subtle)", boxSizing: "border-box", flexShrink: 0, position: "relative" },
  brand: { display: "flex", alignItems: "center", gap: 6, whiteSpace: "nowrap", flexShrink: 0 },
  brandMark: { fontFamily: "var(--font-display)", fontStyle: "italic", fontSize: 17, lineHeight: 1, color: "var(--fg)" },
  brandSupport: { fontSize: 8, fontWeight: 600, letterSpacing: "0.18em", textTransform: "uppercase", color: "var(--fg-dim)", position: "relative", top: 1 },
  spacer: { flex: 1 },
  ...
  panel: { width: 248, flexShrink: 0, borderLeft: "1px solid var(--border-subtle)", background: "var(--bg-panel)", display: "flex", flexDirection: "column", minHeight: 0 },
  ...
  composer: { borderTop: "1px solid var(--border-subtle)", padding: 12, display: "grid", gap: 8, flexShrink: 0 },
  ta: { width: "100%", minHeight: 50, borderRadius: 10, border: "1px solid var(--border)", background: "var(--bg)", color: "var(--fg)", padding: 8, font: "inherit", fontSize: 11, boxSizing: "border-box", resize: "none" },
};

const fBtn = { border: 0, background: "var(--accent)", color: "var(--accent-ink)", borderRadius: 9, padding: "7px 12px", fontFamily: "inherit", fontWeight: 700, fontSize: 11, cursor: "pointer", whiteSpace: "nowrap" };
const gBtn = { border: "1px solid var(--border)", background: "transparent", color: "var(--fg-muted)", borderRadius: 9, padding: "7px 11px", fontFamily: "inherit", fontWeight: 600, fontSize: 11, cursor: "pointer", whiteSpace: "nowrap" };
const iBtn = { width: 28, height: 28, display: "grid", placeItems: "center", border: "1px solid var(--border-subtle)", background: "transparent", color: "var(--fg-dim)", borderRadius: 8, cursor: "pointer", flexShrink: 0, padding: 0 };
...
function Brand() {
  return <div style={v2.brand}><span style={v2.brandMark}>Crux</span><span style={v2.brandSupport}>Editor</span></div>;
}
function AnnotateToggle({ on = true }) {
  return (
    <div style={{ display: "inline-flex", alignItems: "center", gap: 6, color: "var(--fg-dim)", fontSize: 10.5, whiteSpace: "nowrap" }}>
      <span style={{ width: 26, height: 15, borderRadius: 999, background: on ? "var(--accent)" : "var(--border)", position: "relative", flexShrink: 0 }}>
        <span style={{ position: "absolute", top: 2, left: on ? 13 : 2, width: 11, height: 11, borderRadius: 999, background: on ? "var(--accent-ink)" : "var(--fg-faint)" }} />
      </span>
      <span>Annotate</span>
    </div>
  );
...
function ArtifactBody() {
  return (
    <div style={v2.artifact}>
      ...
      <div style={{ ...v2.artH1, outline: "2px solid var(--accent)", outlineOffset: 2 }}>For when a rich editor<br/>is not rich enough.</div>
      <p style={v2.artLede}>Crux opens an agent-generated HTML artifact in a local browser, lets you pinpoint elements, and ships your feedback back to the agent.</p>
    </div>
  );
}
...
function MoreMenu({ open = false }) {
  return (
    <div style={{ position: "relative", flexShrink: 0 }}>
      <button title="More" style={{ ...iBtn, ...(open ? { borderColor: "var(--border-strong)", color: "var(--fg)" } : null) }}><I d={icMore} /></button>
      {open && (
        <div style={{ position: "absolute", right: 0, top: 34, width: 232, background: "var(--bg-panel)", border: "1px solid var(--border)", borderRadius: 12, boxShadow: "var(--shadow-tooltip)", padding: 6, zIndex: 20 }}>
          <div style={{ padding: "7px 9px 8px" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 6, fontFamily: "var(--font-mono)", fontSize: 10.5, color: "var(--fg-muted)" }}>
          ...
          <div style={{ height: 1, background: "var(--border-subtle)", margin: "2px 0 4px" }} />
          {[["Reload artifact", icRefresh], ["Copy DOM snapshot", icCamera]].map(([t, d]) => (
            <div key={t} style={{ display: "flex", alignItems: "center", gap: 8, padding: "7px 9px", borderRadius: 8, fontSize: 11, color: "var(--fg-muted)", cursor: "pointer" }}>
        ...
        <div style={v2.panel}>
          ...
          <div style={v2.composer}>
            ...
            <div style={{ display: "flex", gap: 8, justifyContent: "space-between", alignItems: "center" }}>
              <button style={{ ...gBtn, display: "inline-flex", alignItems: "center", gap: 6, color: "var(--danger)", borderColor: "transparent" }}><I d={icExit} size={13} />End session</button>
        ...
        <div style={v2.panel}>
          ...
          <div style={v2.composer}>
            ...
            <div style={{ display: "flex", justifyContent: "flex-end", position: "relative" }}>
              ...
              <div style={{ position: "absolute", right: 0, bottom: 38, width: 196, background: "var(--bg-panel)", border: "1px solid var(--border)", borderRadius: 11, boxShadow: "var(--shadow-tooltip)", padding: 6, zIndex: 20 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "7px 9px", borderRadius: 8, fontSize: 11, color: "var(--fg-muted)" }}><span style={{ display: "flex", opacity: 0.85 }}><I d={icSend} size={13} /></span>Send to Agent</div>
                <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "7px 9px", borderRadius: 8, fontSize: 11, color: "var(--danger)" }}><span style={{ display: "flex" }}><I d={icExit} size={13} /></span>Send &amp; end session</div>
              </div>
            </div>
          </div>
        </div>
      ...
      <div style={v2.body}>
        ...
        <div style={v2.panel}>
          ...
          <div style={v2.composer}>
            ...
            <button style={{ background: "transparent", border: 0, color: "var(--fg-faint)", fontSize: 10.5, fontFamily: "inherit", cursor: "pointer", padding: 2, textAlign: "center" }}>Send a final note &amp; end session</button>
          </div>
        </div>
      </div>
    </div>
