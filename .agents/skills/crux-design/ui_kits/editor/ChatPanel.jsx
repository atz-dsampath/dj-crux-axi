...
function ChatPanel({ chat, working, pills, draft, onDraftChange, onRemovePill, onSend, onSendAndEnd }) {
  ...
  const s = {
    panel: { borderLeft: "1px solid var(--border-subtle)", background: "var(--bg-panel)", display: "flex", flexDirection: "column", minWidth: 0, minHeight: 0, width: 360, flexShrink: 0 },
    ...
    composer: { display: "grid", gap: 8, padding: "12px 16px", borderTop: "1px solid var(--border-subtle)", minWidth: 0, flexShrink: 0, boxSizing: "border-box" },
    pills: { display: "flex", flexWrap: "wrap", gap: 6, minWidth: 0 },
    ta: { width: "100%", maxWidth: "100%", minWidth: 0, minHeight: 82, resize: "vertical", borderRadius: 12, border: "1px solid var(--border)", background: "var(--bg)", color: "var(--fg)", padding: 10, font: "inherit", fontFamily: "var(--font-sans)", fontSize: 14, boxSizing: "border-box" },
    ...
    sendMain: { border: 0, borderTopLeftRadius: 10, borderBottomLeftRadius: 10, borderTopRightRadius: 0, borderBottomRightRadius: 0, padding: "9px 14px", background: "var(--accent)", color: "var(--accent-ink)", fontFamily: "inherit", fontWeight: 700, cursor: canSend ? "pointer" : "not-allowed", opacity: canSend ? 1 : 0.55, fontSize: 13, whiteSpace: "nowrap" },
    sendCaret: { border: 0, borderTopRightRadius: 10, borderBottomRightRadius: 10, borderTopLeftRadius: 0, borderBottomLeftRadius: 0, padding: "9px 9px", background: "var(--accent)", color: "var(--accent-ink)", cursor: working ? "not-allowed" : "pointer", opacity: working ? 0.55 : 1, borderLeft: "1px solid rgba(23,19,10,.22)", display: "grid", placeItems: "center" },
    menu: { position: "absolute", right: 0, bottom: 44, width: 208, background: "var(--bg-bar)", border: "1px solid var(--border)", borderRadius: 11, boxShadow: "var(--shadow-tooltip)", padding: 6, zIndex: 40 },
  };
