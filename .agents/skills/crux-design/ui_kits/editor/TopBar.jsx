...
function AnnotateSwitch({ on, disabled, onToggle }) {
  const s = {
    wrap: { display: "inline-flex", alignItems: "center", gap: 8, color: "var(--fg-muted)", fontSize: 13, whiteSpace: "nowrap", cursor: disabled ? "not-allowed" : "pointer", opacity: disabled ? 0.55 : 1, userSelect: "none", background: "none", border: 0, fontFamily: "inherit", padding: 0 },
    track: { width: 34, height: 20, borderRadius: 999, background: on ? "var(--accent)" : "var(--border)", position: "relative", flexShrink: 0, transition: "background 150ms var(--ease)" },
    knob: { position: "absolute", top: 2, left: on ? 16 : 2, width: 16, height: 16, borderRadius: 999, background: on ? "var(--accent-ink)" : "var(--fg-faint)", transition: "left 150ms var(--ease)" },
  };
  return (
...
function MenuItem({ icon, label, danger, onClick }) {
  ...
  const base = {
    ...
    border: 0, cursor: "pointer", background: hover ? (danger ? "rgba(240,100,100,.10)" : "var(--border-subtle)") : "transparent",
...
function TopBar({ filePath, annotationOn, onToggleAnnotation, onEndSession, onReload, onSnapshot, ended }) {
  ...
  const s = {
    bar: { height: 56, display: "flex", alignItems: "center", gap: 14, padding: "0 16px", background: "var(--bg-bar)", borderBottom: "1px solid var(--border-subtle)", boxSizing: "border-box", flexShrink: 0 },
    brand: { display: "flex", alignItems: "center", gap: 8, whiteSpace: "nowrap", flexShrink: 0 },
    brandMark: { fontFamily: "var(--font-display)", fontStyle: "italic", fontSize: 22, lineHeight: 1, color: "var(--fg)" },
    brandSupport: { fontFamily: "var(--font-sans)", fontSize: 10, fontWeight: 600, letterSpacing: "0.18em", textTransform: "uppercase", color: "var(--fg-dim)", position: "relative", top: 1 },
    spacer: { flex: 1 },
    moreWrap: { position: "relative", flexShrink: 0 },
    moreBtn: { width: 34, height: 34, display: "grid", placeItems: "center", border: "1px solid", borderColor: menuOpen ? "var(--border-strong)" : "var(--border-subtle)", background: menuOpen ? "var(--border-subtle)" : "transparent", color: menuOpen ? "var(--fg)" : "var(--fg-dim)", borderRadius: 9, cursor: ended ? "not-allowed" : "pointer", padding: 0, opacity: ended ? 0.55 : 1 },
    menu: { position: "absolute", right: 0, top: 42, width: 248, background: "var(--bg-bar)", border: "1px solid var(--border)", borderRadius: 12, boxShadow: "var(--shadow-tooltip)", padding: 6, zIndex: 70 },
    ...
    menuFileBtn: { display: "flex", alignItems: "center", gap: 7, width: "100%", textAlign: "left", border: 0, background: "transparent", padding: "3px 4px", margin: "0 -4px", borderRadius: 7, cursor: "pointer", fontFamily: "var(--font-mono)", fontSize: 12, color: "var(--fg-muted)" },
    menuFileText: { overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", flex: 1 },
    copyHint: { fontFamily: "var(--font-sans)", fontSize: 10, fontWeight: 600, color: copied ? "var(--accent-hover)" : "var(--fg-faint)", flexShrink: 0, display: "flex", alignItems: "center", gap: 4 },
    rule: { height: 1, background: "var(--border)", margin: "4px 0" },
  };

  return (
    <div style={s.bar}>
      <div style={s.brand}>
        <span style={s.brandMark}>Crux</span>
