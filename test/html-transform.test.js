...
import { injectCruxSdk } from "../src/html-transform.js";

test("injects the Crux SDK before the closing body tag", () => {
  const html = "<!doctype html><html><body><h1>Hi</h1></body></html>";
  const result = injectCruxSdk(html, "abc123");
...
test("does not inject Tailwind or DaisyUI design assets so the saved file stays portable", () => {
  ...
  assert.doesNotMatch(result, /data-crux-design/);
});
...
test("appends the Crux SDK when the artifact has no body tag", () => {
  const result = injectCruxSdk("<h1>Hi</h1>", "abc123");
