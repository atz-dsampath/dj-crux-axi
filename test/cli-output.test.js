...
function setupHooksEnv(homeDir, stateDir) {
  ...
  return { ...env, HOME: homeDir, CRUX_AXI_STATE_DIR: stateDir };
}
...
test("home output teaches agents when and how to use Crux Editor", () => {
  const output = createHomeOutput({ bin: `${os.homedir()}/.local/bin/crux-axi`, sessions: [] });

  assert.equal(output.bin, "~/.local/bin/crux-axi");
  assert.match(output.description, /Crux Editor/);
  assert.match(output.description, /complex response/);
  assert.match(output.description, /consider using Crux Editor/);
  ...
  assert.ok(output.help.some((item) => item.includes("crux-axi <html-file>")));
  assert.ok(output.help.some((item) => item.includes("`.crux/`")));
  assert.ok(output.help.some((item) => item.includes("crux-axi playbook <playbook_id>")));
  ...
  assert.ok(!output.help.some((item) => item.includes('<meta name="crux-design" content="off">')));
...
test("the design-priority rule is single-sourced and keeps its three-step semantics", () => {
  ...
  assert.match(DESIGN_SYSTEM_HINT, /crux-axi design/);
...
test("home output warns agents that poll needs an observable wake path", () => {
  const output = createHomeOutput({ bin: "crux-axi", sessions: [] });
  const pollHelp = output.help.find((item) => item.includes("crux-axi poll <html-file>"));
...
test("home output tailors poll guidance when invoked under Codex", () => {
  const output = createHomeOutput({ bin: "crux-axi", sessions: [], agent: "codex" });
  const pollHelp = output.help.find((item) => item.includes("crux-axi poll <html-file>"));
...
test("home output keeps static skill poll guidance safe and agent-neutral", () => {
  const output = createHomeOutput({ bin: "crux-axi", sessions: [], agent: "static" });
  const pollHelp = output.help.find((item) => item.includes("crux-axi poll <html-file>"));
...
test("top-level help renders static home output without dynamic sessions", async () => {
  const stateDir = await mkdtemp(`${os.tmpdir()}/crux-axi-help-test-`);
  try {
    const result = spawnSync(
      process.execPath,
      [fileURLToPath(new URL("../bin/crux-axi.js", import.meta.url)), "--help"],
      {
        cwd: fileURLToPath(new URL("..", import.meta.url)),
        encoding: "utf8",
        env: { ...process.env, CRUX_AXI_STATE_DIR: stateDir },
      },
    );

    assert.equal(result.status, 0, result.stderr || result.stdout);
    assert.match(result.stdout, /playbooks\[7\]/);
    assert.match(result.stdout, /crux-axi playbook <playbook_id>/);
    ...
    assert.match(result.stdout, /crux-axi design/);
    ...
    assert.doesNotMatch(result.stdout, /crux-design/);
...
test("playbook detail output returns focused Crux-native guidance", () => {
  ...
  assert.ok(output.playbook.design_rules.some((item) => item.includes("data-crux-action")));
  assert.ok(output.playbook.design_rules.some((item) => item.includes("data-crux-question")));
  assert.ok(output.playbook.design_rules.some((item) => item.includes("queueKey")));
  assert.ok(output.playbook.crux_notes.some((item) => item.includes("window.crux.queuePrompt")));
  assert.ok(output.playbook.crux_notes.some((item) => item.includes("onsubmit")));
  ...
  assert.ok(output.playbook.crux_notes.some((item) => item.includes("Crux")));
});
...
test("unknown playbook ids produce an actionable validation error", () => {
  assert.throws(
    ...
    (error) => {
      ...
      assert.ok(error.suggestions.some((item) => item.includes("crux-axi playbook")));
      return true;
    },
  );
});
...
test("home directory collapse tolerates Windows mixed separators", () => {
  assert.equal(
    collapseHomeDirectory("C:\\Users\\runneradmin/.local/bin/crux-axi", "C:\\Users\\runneradmin"),
    "~/.local/bin/crux-axi",
  );
  assert.equal(
    collapseHomeDirectory("C:\\Users\\runneradmin\\.local\\bin\\crux-axi", "C:\\Users\\runneradmin"),
...
test("open output keeps the user URL in session data and next_step focused on polling", () => {
  ...
  assert.match(output.next_step, /crux-axi poll \/tmp\/artifact\.html/);
...
test("a user-ended open refuses with a status agents can branch on, not a URL to open", () => {
  ...
  assert.match(output.next_step, /user explicitly ended this Crux Editor session from the browser/);
  ...
  assert.match(output.next_step, /crux-axi \/tmp\/artifact\.html --reopen/);
});
...
test("export output reports the written file and reassures it needs no server", () => {
  ...
  assert.match(output.next_step, /no Crux server/);
...
test("export command writes a portable HTML file next to the artifact", async () => {
  const dir = await mkdtemp(`${os.tmpdir()}/crux-axi-export-test-`);
  ...
  try {
    const result = spawnSync(
      process.execPath,
      [fileURLToPath(new URL("../bin/crux-axi.js", import.meta.url)), "export", artifact],
      {
        cwd: fileURLToPath(new URL("..", import.meta.url)),
        env: { ...process.env, CRUX_AXI_STATE_DIR: dir, CRUX_AXI_TELEMETRY: "0" },
        encoding: "utf8",
      },
    );
...
test("export command treats --out value as an option operand, not the source file", async () => {
  ...
  try {
    const result = spawnSync(
      process.execPath,
      [fileURLToPath(new URL("../bin/crux-axi.js", import.meta.url)), "export", "--out", output, artifact],
      {
        cwd: fileURLToPath(new URL("..", import.meta.url)),
        env: { ...process.env, CRUX_AXI_STATE_DIR: dir, CRUX_AXI_TELEMETRY: "0" },
        encoding: "utf8",
...
test("share output reports the public url and the secret update key", () => {
  ...
  assert.match(output.next_step, /ht-ml\.app \(https:\/\/ht-ml\.app\), a third-party host not part of Crux/);
});
...
test("share command publishes the artifact to ht-ml.app and returns the public url", async () => {
  const dir = await mkdtemp(`${os.tmpdir()}/crux-axi-share-test-`);
  const artifact = `${dir}/report.html`;
  ...
  try {
    ...
    const child = spawn(
      process.execPath,
      [fileURLToPath(new URL("../bin/crux-axi.js", import.meta.url)), "share", "--password", "pw", artifact],
      {
        cwd: fileURLToPath(new URL("..", import.meta.url)),
        env: {
          ...process.env,
          CRUX_AXI_STATE_DIR: dir,
          CRUX_AXI_TELEMETRY: "0",
          CRUX_AXI_HTML_APP_API_URL: `http://127.0.0.1:${htmlApp.port}`,
        },
      },
    );
    let stdout = "";
...
test("share command treats a whitespace-only password as public", async () => {
  ...
  try {
    const child = spawn(
      process.execPath,
      [fileURLToPath(new URL("../bin/crux-axi.js", import.meta.url)), "share", "--password", "   ", artifact],
      {
        cwd: fileURLToPath(new URL("..", import.meta.url)),
        env: {
          ...process.env,
...
test("share help distinguishes public default from password-protected shares", () => {
  ...
  const home = createHomeOutput({ bin: "crux-axi", sessions: [] });
  const homeShareHelp = home.help.find((item) => item.includes("crux-axi share <html-file>"));
...
test("a poll reporting the session ended by the user tells the agent to stop and not reopen", () => {
  ...
  assert.match(output.next_step, /user ended this Crux Editor session/);
  assert.match(output.next_step, /Stop polling/);
  assert.match(output.next_step, /do not run `crux-axi \/tmp\/report\.html` to reopen it/);
  assert.match(output.next_step, /deliver any remaining updates directly in this conversation/i);
  assert.match(output.next_step, /crux-axi \/tmp\/report\.html --reopen/);
});
...
test("a poll reporting an agent-ended session allows a plain reopen if still needed", () => {
  ...
  assert.match(output.next_step, /Stop polling/);
  assert.match(output.next_step, /crux-axi \/tmp\/report\.html`\s+to open a fresh session/);
...
test("the final feedback batch before an agent end preserves ended_by and allows plain reopen", () => {
  ...
  assert.equal(output.session.session_ended, true);
  assert.equal(output.session.ended_by, "agent");
  assert.match(output.next_step, /last feedback before the Crux Editor session ended/);
  assert.match(output.next_step, /crux-axi \/tmp\/report\.html`\s+to open a fresh session/);
  assert.doesNotMatch(output.next_step, /--reopen/);
  assert.doesNotMatch(output.next_step, /user ended this Crux Editor session/);
});
...
test("final user-ended feedback still requires severe layout repair without reopening", () => {
  ...
  assert.match(output.next_step, /without reopening this ended Crux session/);
  assert.doesNotMatch(output.next_step, /--reopen/);
});
...
test("spawned poll with piped stderr banners once and leaves re-run guidance when killed", async () => {
  const stateDir = await mkdtemp(`${os.tmpdir()}/crux-axi-poll-wait-test-`);
  ...
  try {
    ...
    const child = spawn(
      process.execPath,
      [fileURLToPath(new URL("../bin/crux-axi.js", import.meta.url)), "poll", artifact],
      {
        cwd: fileURLToPath(new URL("..", import.meta.url)),
        env: { ...process.env, CRUX_AXI_STATE_DIR: stateDir, CRUX_AXI_PORT: String(server.port) },
      },
    );
...
test("waiting next step reassures agents that re-running poll loses nothing", () => {
  ...
  assert.match(output.next_step, /crux-axi poll \/tmp\/report\.html/);
  assert.match(output.next_step, /without --timeout-ms/);
  assert.match(output.next_step, /queued feedback is never lost/);
});
...
test("setup hooks resolves HOME before platform-specific user profile variables", () => {
  assert.equal(
    resolveHookHomeDir({ HOME: "/tmp/crux-home", USERPROFILE: "C:\\Users\\runneradmin" }, "/fallback"),
    "/tmp/crux-home",
  );
});
...
test("setup hooks creates a Copilot CLI hook that injects additional context", () => {
  ...
  assert.match(updated.hooks.sessionStart[1].bash, /crux-axi/);
...
test("Copilot CLI ambient context script wraps crux output as hook JSON", async () => {
  const tempDir = await mkdtemp(`${os.tmpdir()}/crux-axi-copilot-hook-`);
  try {
    const fakeCli = path.join(tempDir, "fake-crux.js");
    ...
    assert.match(output.additionalContext, /## AXI ambient context: crux-axi/);
...
test("setup hooks installs agent session hooks explicitly", async () => {
  const stateDir = await mkdtemp(`${os.tmpdir()}/crux-axi-setup-state-`);
  const homeDir = await mkdtemp(`${os.tmpdir()}/crux-axi-setup-home-`);
  try {
    const result = spawnSync(
      process.execPath,
      [fileURLToPath(new URL("../bin/crux-axi.js", import.meta.url)), "setup", "hooks"],
      {
        cwd: fileURLToPath(new URL("..", import.meta.url)),
        encoding: "utf8",
    ...
    assert.ok(existsSync(`${homeDir}/.copilot/hooks/crux-axi.json`));

    const copilotHook = JSON.parse(await readFile(`${homeDir}/.copilot/hooks/crux-axi.json`, "utf8"));
...
test("setup hooks exits with an error when hook installation fails", async () => {
  const stateDir = await mkdtemp(`${os.tmpdir()}/crux-axi-setup-fail-state-`);
  const homeDir = await mkdtemp(`${os.tmpdir()}/crux-axi-setup-fail-home-`);
  try {
...
test("server entry resolves to a node-executable script that actually invokes run()", () => {
  // Running from source, the entry must be `bin/crux-axi.js` (the only file in the
  ...
  // From source: bin/crux-axi.js is present and preferred.
  assert.equal(entry, fileURLToPath(new URL("../bin/crux-axi.js", import.meta.url)));
});
...
test("local built CLI opens force a server restart while source and installed runs do not", () => {
  ...
  assert.equal(shouldForceRestartForLocalBuild(`${root}/bin/crux-axi.js`, true), false);
  assert.equal(shouldForceRestartForLocalBuild("/usr/local/lib/node_modules/crux-axi/dist/cli.mjs", false), false);
});
...
test("shouldKillProcessOnPort kills pre-handshake Crux servers after shutdown fails", () => {
  assert.equal(shouldKillProcessOnPort("0.1.4", { ok: true }), true);
});

test("shouldKillProcessOnPort only kills Crux servers with a mismatched version", () => {
  assert.equal(shouldKillProcessOnPort("0.1.4", { ok: true, app: "crux-axi", version: "0.1.3" }), true);
  assert.equal(shouldKillProcessOnPort("0.1.4", { ok: true, app: "crux-axi", version: "0.1.4" }), false);
});

test("shutdownServerOnPort kills pre-handshake Crux servers when shutdown does not free the port", async () => {
  let shutdowns = 0;
  let kills = 0;
...
test("shutdownServerOnPort ignores unidentified health responders", async () => {
  ...
  const output = await shutdownServerOnPort(4387, {
    ...
    processMatchesCrux: () => false,
  });

  assert.equal(shutdowns, 0);
  assert.equal(kills, 0);
  assert.deepEqual(output, { server: { status: "not-crux", port: 4387 } });
});
...
test("open can resume a session without opening another browser window", () => {
  ...
  assert.equal(shouldOpenBrowser(["artifact.html"], { CRUX_AXI_NO_OPEN: "1" }), false);
  ...
  assert.match(getCommandHelp("design"), /crux-axi design/);
...
test("polling a file without an active session tells the agent to open it first", () => {
  assert.throws(
    ...
    (error) => {
      ...
      assert.match(error.message, /No active Crux Editor session/);
      assert.ok(error.suggestions.some((item) => item.includes("crux-axi /tmp/report.html")));
      return true;
    },
  );
});

test("network fetch failures become structured Crux server errors", async () => {
  await assert.rejects(
    () => fetchJson("http://127.0.0.1:1/api/poll"),
    (error) => {
      assert.ok(error instanceof AxiError);
      assert.equal(error.code, "SERVER_ERROR");
      assert.match(error.message, /Crux Editor server connection failed/);
      assert.ok(error.suggestions.some((item) => item.includes("crux-axi server --verbose")));
      return true;
    },
  );
});
  ...
  try {
    ...
    await assert.rejects(
      ...
      (error) => {
        assert.ok(error instanceof AxiError);
        assert.equal(error.code, "SERVER_ERROR");
        assert.match(error.message, /Crux Editor poll response was interrupted/);
        return true;
      },
    );
...
test("stop command shuts down the running server on the configured port", async () => {
  const dir = await mkdtemp(`${os.tmpdir()}/crux-axi-stop-test-`);
