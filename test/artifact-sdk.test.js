...
function matchesSelector(el, selector) {
  ...
  if (selector === "[data-crux-question]") return el.getAttribute("data-crux-question") !== null;
...
test("deriveCruxQueueKey uses explicit queueKey first", () => {
  const input = node("input", { type: "radio", name: "plan" });

  assert.equal(deriveCruxQueueKey(input, { queueKey: "deployment-plan" }), "deployment-plan");
});

test("deriveCruxQueueKey allows explicit empty queueKey to suppress derivation", () => {
  const button = node("button");
  node("section", { "data-crux-question": "deployment-plan" }, [button]);

  assert.equal(deriveCruxQueueKey(button, { queueKey: "" }), "");
});

test("deriveCruxQueueKey groups controls inside data-crux-question", () => {
  const first = node("button");
  const second = node("button");
  node("section", { "data-crux-question": "deployment-plan" }, [first, second]);

  assert.equal(deriveCruxQueueKey(first), "question:deployment-plan");
  assert.equal(deriveCruxQueueKey(second), "question:deployment-plan");
});

test("deriveCruxQueueKey groups radio options by scoped group name", () => {
  ...
  assert.equal(deriveCruxQueueKey(planA), "radio:form:deploy:plan");
  assert.equal(deriveCruxQueueKey(planB), "radio:form:deploy:plan");
});

test("deriveCruxQueueKey keeps same radio names independent across scopes", () => {
  ...
  assert.notEqual(deriveCruxQueueKey(first), deriveCruxQueueKey(second));
});

test("deriveCruxQueueKey does not infer plain button grouping without question metadata", () => {
  const button = node("button");

  assert.equal(deriveCruxQueueKey(button), "");
});

test("deriveCruxQueueKey keys checkbox toggles per checkbox, not per group", () => {
...
test("deriveCruxQueueKey does not collide checkbox default values", () => {
...
test("deriveCruxQueueKey keys named selects as fields", () => {
  const select = node("select", { name: "region" });
  node("form", { id: "deploy" }, [select]);

  assert.equal(deriveCruxQueueKey(select), "field:form:deploy:region");
});
